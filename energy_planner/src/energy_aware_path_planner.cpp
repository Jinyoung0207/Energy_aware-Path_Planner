#include "energy_aware_path_planner.hpp"
#include <algorithm>
#include <cmath>
#include <limits>
#include <iomanip>
...
namespace energy_planner 
{
    ...
    EnergyPathPlanner::EnergyPathPlanner(ros::NodeHandle& nh)
        : nh_(nh)
    {
        ...
        ros::NodeHandle pnh("~");
        pnh.param("e_grd_jpm", e_grd_jpm_, e_grd_jpm_);
        pnh.param("e_air_jpm", e_air_jpm_, e_air_jpm_);
        pnh.param("v_grd_mps", v_grd_mps_, v_grd_mps_);
        pnh.param("v_air_mps", v_air_mps_, v_air_mps_);
        pnh.param("w_e", w_e_, w_e_);
        pnh.param("w_t", w_t_, w_t_);
        pnh.param("conflict_radius_m", conflict_radius_m_, conflict_radius_m_);
        pnh.param("goal_hold_s", goal_hold_s_, goal_hold_s_);
        pnh.param("time_bin_s", time_bin_s_, 0.2);
        pnh.param("allow_wait", allow_wait_, true);
        pnh.param("e_wait_grd_jps", e_wait_grd_jps_, e_wait_grd_jps_);
        pnh.param("e_hover_jps", e_hover_jps_, e_hover_jps_);
        pnh.param("gamma_air_eff_", gamma_air_eff_, 1.0); 
        pnh.param<std::string>("octomap_topic", octomap_topic_, "/octomap_binary");


        voxel_sub_ = nh_.subscribe(octomap_topic_, 1, &EnergyPathPlanner::octomapCallback, this);
    ...
    geometry_msgs::Point EnergyPathPlanner::gridToWorld(const GridIdx& idx) const 
    {
    ...
    PlanningManager::PlanningManager(ros::NodeHandle& nh, ros::NodeHandle& pnh)
        : nh_(nh), pnh_(pnh), planner_(nh)
    {
      pnh_.param("energy_aware_path_planner/common/publish_topics", publish_topics_, true);
      pnh_.param("energy_aware_path_planner/common/exit_after_all", exit_after_all_, true);
      pnh_.param<std::string>("energy_aware_path_planner/common/reservation_param_root", res_root_, "/reservations");
    }

    bool PlanningManager::waitForMap() 
    {
        ROS_INFO("Waiting for Map...");
        while (ros::ok() && !planner_.isMapReady()) 
        {
            ros::spinOnce();
    ...
    void PlanningManager::publishPath(const std::string& name, const TimedPath& tp) 
    {
        ...
        {
            pubs[topic] = nh_.advertise<nav_msgs::Path>(topic, 1, true);
        }
        pubs[topic].publish(msg);
    }

    static bool parseVec3(XmlRpc::XmlRpcValue& node, const char* key, std::array<double,3>& out)
    {
        if (!node.hasMember(key)) return false;
        auto& a = node[key];
        if (a.getType()!=XmlRpc::XmlRpcValue::TypeArray || a.size()!=3) return false;
        out = { (double)a[0], (double)a[1], (double)a[2] };
        return true;
    }
    ...
    void PlanningManager::loadAndRun() 
    {
        ...
        // 1. Load Robot Configs
        if (pnh_.getParam("energy_aware_path_planner/robots", list) && list.getType()==XmlRpc::XmlRpcValue::TypeArray)
        {
            for (int i=0; i<list.size(); ++i) 
            {
                auto& r = list[i];
                if (r.getType()!=XmlRpc::XmlRpcValue::TypeStruct) continue;
                RobotCfg cfg;
                if (r.hasMember("name")) cfg.name = static_cast<std::string>(r["name"]);
                if (r.hasMember("priority")) cfg.priority = (int)r["priority"];
                parseVec3(r, "start_xyz", cfg.start_xyz);
                parseVec3(r, "goal_xyz", cfg.goal_xyz);
                
                if (r.hasMember("params")) 
                {
                     auto& pr = r["params"];
                     for (auto it = pr.begin(); it != pr.end(); ++it) 
                     {
                        if (it->second.getType() == XmlRpc::XmlRpcValue::TypeDouble) 
                            cfg.f64[it->first] = (double)it->second;
                        else if (it->second.getType() == XmlRpc::XmlRpcValue::TypeInt) // Int형도 double로 받아주기
                            cfg.f64[it->first] = (double)(int)it->second;
                     }
                }
        ...
        {
            ROS_WARN("No 'robots' param found. Using Default Test Robot.");
        ...
        {
            ROS_INFO("Planning for %s [P:%d]...", bot.name.c_str(), bot.priority);
            applyParams(bot);
            ...
            if (res) 
            {
                ...
                ROS_INFO("Success! Travel Time: %.2fs (Smoothed)", smoothed_path.back().t - smoothed_path.front().t);
            } 
            else 
            {
                ROS_WARN("Failed to find path for %s", bot.name.c_str());
            }
        }
...
{
    ros::init(argc, argv, "energy_path_planner");
    ros::NodeHandle nh;
    ros::NodeHandle pnh("~");

    energy_planner::PlanningManager manager(nh, pnh);
