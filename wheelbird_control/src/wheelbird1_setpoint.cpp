...
{
    mode_flag_sub = nh.subscribe<std_msgs::Int64>("/wheelbird1/mode_flag", 1, &multi_setpoint_ctrl::controller_mode_num_Callback, this);
    
    current_pose_1_sub = nh.subscribe<geometry_msgs::PoseStamped>("/wheelbird1/mavros/local_position/pose", 1, &multi_setpoint_ctrl::pose1Callback, this);
    ref_1_sub = nh.subscribe<nav_msgs::Path>("/wheelbird1/ref_trajectory", 1, &multi_setpoint_ctrl::ref_1_Callback, this);
    ...
    state1_sub = nh.subscribe<mavros_msgs::State>("/wheelbird1/mavros/state", 1, &multi_setpoint_ctrl::state1Callback, this);
    ...
    param_set_client1 = nh.serviceClient<mavros_msgs::ParamSet>("/wheelbird1/mavros/param/set");// Auto landing speed
    param_th_client1 = nh.serviceClient<mavros_msgs::ParamSet>("/wheelbird1/mavros/param/set"); // Auto landing vertical velocity threshold

    set_point_1_pub = nh.advertise<geometry_msgs::PoseStamped>("/wheelbird1/mavros/setpoint_position/local", 1);
    pose_point_1_pub = nh.advertise<geometry_msgs::PointStamped>("/wheelbird1/pose_point", 1);
...
{
    ros::init(argc, argv, "wheelbird1_setpoint_node");
    ros::NodeHandle nh;
